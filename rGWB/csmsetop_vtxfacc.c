...
struct i_neighborhood_t
{
    ...
    enum csmsetop_classify_resp_solid_t prev_position;
    enum csmsetop_classify_resp_solid_t recl_position;
};
                        ...
                        const struct csmtolerance_t *tolerances))
{
    ...
    do
    {
        ...
        if (csmopbas_is_wide_hedge(hedge_iterator, tolerances, &Ux_bisec, &Uy_bisec, &Uz_bisec) == CSMTRUE)
        {
            ...
            hedge_neighborhood_wide = i_create_neighborhod(hedge_iterator, cl_resp_plane, cl_resp_plane);
                        ...
                        const struct csmtolerance_t *tolerances)
{
    ...
    if (hedge_neighborhood->prev_position == CSMSETOP_CLASSIFY_RESP_SOLID_ON && next_hedge_neighborhood->prev_position == CSMSETOP_CLASSIFY_RESP_SOLID_ON)
    {
                ...
                case CSMSETOP_A_VS_B:
                    ...
                    switch (set_operation)
                    {
                        case CSMSETOP_OPERATION_UNION:
                            ...
                            hedge_neighborhood->recl_position = CSMSETOP_CLASSIFY_RESP_SOLID_OUT;
                            next_hedge_neighborhood->recl_position = CSMSETOP_CLASSIFY_RESP_SOLID_OUT;
                            break;
                            
                        case CSMSETOP_OPERATION_INTERSECTION:
                        case CSMSETOP_OPERATION_DIFFERENCE:
                            
                            hedge_neighborhood->recl_position = CSMSETOP_CLASSIFY_RESP_SOLID_IN;
                            next_hedge_neighborhood->recl_position = CSMSETOP_CLASSIFY_RESP_SOLID_IN;
                            break;
                        ...
                        struct i_neighborhood_t *next_hedge_neighborhood)
{
    ...
    if (hedge_neighborhood->prev_position == CSMSETOP_CLASSIFY_RESP_SOLID_ON)
    {
        enum csmsetop_classify_resp_solid_t new_position;
        
        if (prev_hedge_neighborhood->prev_position == CSMSETOP_CLASSIFY_RESP_SOLID_IN && next_hedge_neighborhood->prev_position == CSMSETOP_CLASSIFY_RESP_SOLID_IN)
        {
            new_position = CSMSETOP_CLASSIFY_RESP_SOLID_IN;
        }
        else if ((prev_hedge_neighborhood->prev_position == CSMSETOP_CLASSIFY_RESP_SOLID_IN && next_hedge_neighborhood->prev_position == CSMSETOP_CLASSIFY_RESP_SOLID_OUT)
                    || (prev_hedge_neighborhood->prev_position == CSMSETOP_CLASSIFY_RESP_SOLID_OUT && next_hedge_neighborhood->prev_position == CSMSETOP_CLASSIFY_RESP_SOLID_IN))
        {
        ...
        {
            assert(prev_hedge_neighborhood->prev_position == CSMSETOP_CLASSIFY_RESP_SOLID_OUT && next_hedge_neighborhood->prev_position == CSMSETOP_CLASSIFY_RESP_SOLID_OUT);
    ...
    if (csmdebug_debug_enabled() == CSMTRUE)
    {
        ...
        for (i = 0; i < num_sectors; i++)
        {
            ...
            csmdebug_print_debug_info(
                        ...
                        i_position_to_string(hedge_neighborhood->recl_position));
        }
                        ...
                        csmArrayStruct(csmedge_t) *set_of_null_edges, csmArrayStruct(csmedge_t) *set_of_null_edges_other_solid)
{
    ...
    i_reclassify_on_sectors_vertex_neighborhood(A, B, C, set_operation, a_vs_b, tolerances, vertex_neighborhood);
